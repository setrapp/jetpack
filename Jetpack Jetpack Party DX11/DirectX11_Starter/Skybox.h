...
public:
	Skybox()
	{
		HR(CreateDDSTextureFromFile(DXConnection::Instance()->device, L"../Assets/Textures/skybox.dds", &skyboxResource, &skyboxShaderView));
		D3D11_SHADER_RESOURCE_VIEW_DESC p;
		skyboxShaderView->GetDesc(&p);
		
		bool result;

		// Material settings
		if (!m_shader) m_shader = AssetManager::Instance()->GetPixelShader("texture");
		...
		if (!m_model)
		{
			m_model = AssetManager::Instance()->GetModel("cube");
		}
