...
public class DialogueVolSlider : MonoBehaviour {
	...
	void Start () {
		dialogueList = GameObject.FindGameObjectsWithTag("Audio_Dialogue");
	}
	...
	void OnMouseDrag()
	{
		...
		for(int i = 0; i < dialogueList.Length; i++){
			dialogueList[i].GetComponent<ID_AudioDialogue>().UpdateDialogueVolumeLevel(percentage/3);
		}
	}
}
